...
DOWNLOAD_LINK="https://www.hep.ucl.ac.uk/uhen/libRootFftwWrapper/releases/libRootFftwWrapper-1.10.tar.gz"
...
if [ $SKIP_DOWNLOAD = false ]; then
	...
	wget --no-check-certificate "$DOWNLOAD_LINK" -O "$PACKAGE_DIR_NAME.tar.gz" || exit 11
...
#set(FFTW_LIBRARIES "$ENV{FFTWSYS}/lib/libfftw3.so.3.3.4")\
#set(FFTW_INCLUDES "$ENV{FFTWSYS}/include"):' CMakeLists.txt
#	sed -i 's:@ccmake:@cmake:' Makefile
#	make configure "$MAKE_ARG" || exit 31
	# Fix Makefile for separate ROOT include and library directories
	sed -i 's:^CXXFLAGS     += $(ROOTCFLAGS) $(SYSINCLUDES):CXXFLAGS     += $(ROOTCFLAGS) $(SYSINCLUDES) -L$(UTIL_LIB_DIR) -I$(UTIL_INC_DIR):' Makefile
