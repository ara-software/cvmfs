...
usage() {
	echo "usage: $0 [-h] [-d destination] [-s destination] [-b destination] [-r destination] [--skip_download, --skip_build] [--clean_source]"
...
if [ $SKIP_BUILD = false ]; then
	...
	sed -i 's:add_subdirectory(utilities/repeder):#add_subdirectory(utilities/repeder):' CMakeLists.txt
